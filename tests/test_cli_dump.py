...
def test_cli_dump_stdout(tmp_path):
    # We can't easily inject a mock orchestrator into the CLI process without complex patching.
    # However, we can run the CLI against a temp directory by modifying the CLI to accept a root dir,

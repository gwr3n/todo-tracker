...
def test_dump_history_scenario(tmp_path):
    ...
    print(f"  - Task A (pending): {[t for t in tasks_to_dump if t['description'] == 'Task A' and t['status'] == 'pending'][0]['id'][:8]}")
    print(f"  - Task A (completed): {[t for t in tasks_to_dump if t['description'] == 'Task A' and t['status'] == 'completed'][0]['id'][:8]}")
    print(f"  - Task B (pending): {[t for t in tasks_to_dump if t['description'] == 'Task B'][0]['id'][:8]}")
